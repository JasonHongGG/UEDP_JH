...
pub fn get_packages(state: State<'_, AppState>) -> Result<Vec<PackageInfo>, String> {
    ...
    for entry in obj_mgr.cache_by_address.iter() {
        let obj = entry.value();
        let pkg_name = obj.full_name.split(|c| c == '.' || c == ':').next().unwrap_or("").to_string();
        if !pkg_name.is_empty() && pkg_name.starts_with("/Script/") {
...
pub fn get_objects(state: State<'_, AppState>, package_name: String, category: String) -> Result<Vec<ObjectSummary>, String> {
    ...
    for entry in obj_mgr.cache_by_address.iter() {
        let obj = entry.value();
        let pkg_name = obj.full_name.split(|c| c == '.' || c == ':').next().unwrap_or_default();
...
pub fn get_handlers() -> impl Fn(tauri::ipc::Invoke) -> bool {
    tauri::generate_handler![fetch_system_processes, attach_to_process, get_ue_version, get_fname_pool_address, parse_fname_pool, parse_guobject_array, get_guobject_array_address, get_gworld_address, show_base_address, get_packages, get_objects, get_object_details]
}
